...
function apiUrl(repo: GiteeRepoRef, path: string): URL {
  return new URL(`${repo.apiBaseUrl.replace(/\/+$/, '')}${path}`)
}
...
export function isGiteeReviewCreationAuthenticated(): boolean {
  return Boolean(resolveGiteeAuthConfig().accessToken)
}

function classifyCreateError(error: unknown): CreateHostedReviewResult {
  const message = apiErrorMessage(error)
  if (message) {
    console.warn('createGiteePullRequest failed:', message)
  }
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Gitee is not authenticated. Connect Gitee in Settings → Review providers → Gitee.'
    }
  }
...
async function findExistingPullRequest(
  ...
  const existing = await getGiteePullRequestForBranch(repoPath, head, null, connectionId)
...
export async function createGiteePullRequest(
  ...
  const repo = await getGiteeRepoSlug(repoPath, connectionId)
  if (!repo) {
    return {
      ok: false,
      code: 'unsupported_provider',
  ...
  const body =
    input.useTemplate && !input.body?.trim()
      ...
      : (input.body ?? '')
  const config: GiteeAuthConfig = resolveGiteeAuthConfig()
  const requestBody = {
    base,
    head,
    title,
  ...
  try {
    const raw = await requestHostedReviewJson<RawGiteePull>(
    ...
    const created = mapGiteePull(raw)
    if (created) {
