...
import { CalendarPersistence, installCalendarPersistenceContext } from './calendar-persistence'
import {
...
export function installStoreDomainContexts(target: object, domains: StoreDomains): void {
  ...
  installCalendarPersistenceContext(target, domains.calendar)
...
export function createStoreDomains(runtime: StoreRuntimeState): StoreDomains {
  ...
  const calendar = new CalendarPersistence(runtime, scheduling)
