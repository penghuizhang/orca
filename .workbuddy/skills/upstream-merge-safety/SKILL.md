...
**直接用 `/orca-upstream-sync`**（`.agents/skills/orca-upstream-sync/`）跑完整流程——它按步骤引用本
skill 的两个脚本。本 skill 是**正确性判据与原理**的落点：静默重复分布表、冲突取舍规则、实测优先
裁决、脚本为何这么写。同理，`orca-dev-workflow` 只管打包与安装。
